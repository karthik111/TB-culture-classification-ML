...
clf = mixture.GaussianMixture(n_components=int(N_COMP), covariance_type='full', verbose=1)
g_pos = clf.fit(x_train)
...
print("starting GMM for condensation..")
x_raw_train_input = np.array(list(samples_conden.values()))
g_conden = get_gmm_model(x_raw_train_input, N_COMP = 15)

print("starting GMM for contamination..")
x_raw_train_input = np.array(list(samples_contam.values()))
g_contam = get_gmm_model(x_raw_train_input, N_COMP = 15)
